...
uint8_t DS18B20_ROM_ID[2][8] = { 0x28, 0xD3, 0x60, 0x71, 0x00, 0x00, 0x00, 0xB5,
								 0x28, 0xB8, 0x63, 0x71, 0x00, 0x00, 0x00, 0xC0 };
...
float DS18B20_Get_Temperature(int DS18B20_No)
{
	...
	// 开启温度转换
	DS18B20_Init();				// 1.初始化
	DS18B20_WriteByte(0x55);	// 2.ROM操作指令, 匹配ROM指令
	for (i = 0; i < 8; ++i)		// 3.发送ROM的ID
	{
		DS18B20_WriteByte(DS18B20_ROM_ID[DS18B20_No][i]);
	}
	DS18B20_WriteByte(0x44);	// 4.温度转换指令
	
	// 读取温度值
	DS18B20_Init();				// 1.初始化
	DS18B20_WriteByte(0x55);	// 2.ROM操作指令, 忽略ROM指令
	for (i = 0; i < 8; ++i)		// 3.发送ROM的ID
	{
		DS18B20_WriteByte(DS18B20_ROM_ID[DS18B20_No][i]);
	}
	DS18B20_WriteByte(0xBE);	// 4.读暂存器指令
...
void DS18B20_ReadROM_ID(void)
{
	...
	// 读DS18B20的ROM
	DS18B20_Init();				// 1.初始化
	DS18B20_WriteByte(0x33);	// 2.ROM操作指令, 读取ROM指令
