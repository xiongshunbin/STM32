...
/*
 * 返回值 = 1, 代表已经配置过RTC, 则无需重新配置时间;
 * 返回值 = 0, 表示未配置时间, 则需要指定时间。
*/
int RTC_Configuration(void)
{
	...
	if (BKP_ReadBackupRegister(BKP_DR1) != 0xABCD)
	{
		...
		do
		{
			...
			flag = UART1_Scanf(&time.minute, 2);
		} while(flag);
		printf("Minute = %d\n", time.minute);
		...
		do
		{
			...
			flag = UART1_Scanf(&time.second, 2);
		} while(flag);
		printf("Second = %d\n", time.second);
		
		Set_Time();
		
		// 12.标记后备数据寄存器1为0xABCD
...
void Set_Time(void)
{
	...
	{
		time_counter += (days_in_year(i)) * 86400;	// 1天 = 86400秒
	}
	...
	time_counter += (time.day - 1) * 86400;
	time_counter += (time.hour * 60 + time.minute) * 60 + time.second;
